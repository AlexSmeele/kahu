...
import { MOCK_TROUBLESHOOTING_TOPICS } from '@/lib/mockData';
...
export default function TroubleshootingDetailPage() {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6">
        ...
        <p className="text-muted-foreground mb-4">
          We couldn't find the troubleshooting topic you're looking for.
        </p>
  ...
  const IconComponent = (LucideIcons as any)[topic.icon] || LucideIcons.AlertTriangle;
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              ...
              Solutions ({topic.subSessions.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {topic.subSessions.map((lesson: any, index: number) => (
              <Card
                key={lesson.id}
                className="cursor-pointer hover:shadow-md transition-all active:scale-[0.98]"
                onClick={() => navigate(`/training/troubleshooting/${topicId}/${lesson.id}/instructional`)}
              >
            ...
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-sm text-foreground leading-relaxed">
                  <strong>Problem-Solving Guide:</strong> These solutions help you address common behavioral issues. 
                  Remember, patience and consistency are key. If problems persist, consult a professional trainer or veterinarian.
                </p>
              </div>
            </div>
          </CardContent>
