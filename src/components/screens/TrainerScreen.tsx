  ...
  const sendMessage = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('ai-trainer', {
        body: {
  ...
  return <div className="flex flex-col h-full">
      ...
      <div className="flex flex-col items-center justify-center p-6 text-center h-full">
            <div className="w-full max-w-sm mb-8">
              <img src={heroImage} alt="Happy dog in training session" className="w-full h-48 object-cover rounded-2xl shadow-[var(--shadow-medium)]" />
            </div>
            ...
            <div className="w-full max-w-md">
              ...
              <div className="space-y-2">
                {[`My dog ${currentDog?.name || ''} won't come when called`, "Help with house training", "Teaching my puppy to sit"].map((prompt, index) => <button key={index} onClick={() => sendSuggestedPrompt(prompt)} className="w-full text-left p-3 bg-secondary rounded-lg text-sm text-secondary-foreground hover:bg-secondary-hover transition-colors">
                    "{prompt}"
                  </button>)}
              </div>
            </div>
      ...
      <div className="p-4 space-y-4">
            {messages.map(message => <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <Card className={`max-w-[80%] p-3 ${message.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-card border-border'}`}>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 bg-card/95 backdrop-blur-sm border-t border-border safe-bottom">
        <div className="flex gap-2 items-end">
          <div className="flex-1">
            <Input value={inputMessage} onChange={handleInputChange} onKeyPress={handleKeyPress} placeholder={`Ask about training ${currentDog?.name || 'your dog'}...`} className="resize-none" disabled={isLoading} />
          </div>
          <Button onClick={sendMessage} disabled={!inputMessage.trim() || isLoading} className="btn-primary px-0">
