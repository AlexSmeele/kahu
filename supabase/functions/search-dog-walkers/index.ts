...
async function searchGooglePlaces(query: string, latitude?: number, longitude?: number, apiKey?: string): Promise<WalkerSearchResult[]> {
  if (!apiKey) {
  ...
  try {
    const searchQuery = `dog walking service ${query}`;
    ...
    console.log('Calling Google Places API for dog walkers');
    ...
    const results: WalkerSearchResult[] = data.results.map((place: any) => ({
      name: place.name,
      business_name: place.name,
      address: place.formatted_address || place.vicinity,
      service_area: place.vicinity,
    ...
    console.log(`Found ${results.length} dog walkers from Google Places`);
    return results;
  } catch (error) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { query, latitude, longitude, service_area } = await req.json();

    console.log('Searching for dog walkers:', { query, latitude, longitude, service_area });
    ...
    const dbResults: WalkerSearchResult[] = (dbWalkers || []).map((walker: any) => ({
      ...walker,
      source: 'database' as const,
      distance: latitude && longitude && walker.latitude && walker.longitude
        ? calculateDistance(latitude, longitude, walker.latitude, walker.longitude)
        : undefined,
    }));
    ...
    console.log(`Found ${allResults.length} total dog walkers (${dbResults.length} database, ${googleResults.length} Google)`);
