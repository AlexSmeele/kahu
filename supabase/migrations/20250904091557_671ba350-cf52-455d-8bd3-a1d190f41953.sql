...
ALTER TABLE tricks DROP CONSTRAINT IF EXISTS tricks_category_check;

-- Add new category constraint with comprehensive categories
ALTER TABLE tricks ADD CONSTRAINT tricks_category_check 
CHECK (category IN (
  'Obedience', 'Performance', 'Foundation / Prop Work', 'Prop Work', 
  'Body Control', 'Impulse Control', 'Practical', 'Chain',
  'basic', 'intermediate', 'fun'  -- Keep old categories for compatibility
));

-- Clear existing tricks and populate with comprehensive list
DELETE FROM tricks;

-- Insert all tricks with proper difficulty levels and time estimates
INSERT INTO tricks (name, description, instructions, difficulty_level, category, prerequisites, estimated_time_weeks) VALUES

-- Beginner Tricks (Level 1-2)
